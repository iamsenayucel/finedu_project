...
export default function RiskReturnTradeoff({ onComplete }: RiskReturnTradeoffProps) {
  ...
  // ── GİRİŞ: büyük sözlük + strateji inceleme ekranı (sayfalı) ────────────────
  if (stage === 'intro') {
    const introPanel = MIRAS_PANELS[studyPage];
    const isLastPage = studyPage === MIRAS_PANELS.length - 1;
    const startGame = () => {
      setLeavingIntro(true);
      setTimeout(() => setStage('playing'), 380);
    };
    const nextPage = () => setStudyPage(p => Math.min(p + 1, MIRAS_PANELS.length - 1));
          ...
          <>
            ...
            <div className="flex items-center justify-center gap-4 mb-5">
              ...
              <div className="flex gap-1.5">
                {MIRAS_PANELS.map((_, i) => (
                  <button
                    key={i}
                    onClick={() => setStudyPage(i)}
                    className={`h-2 rounded-full transition-all ${i === studyPage ? 'bg-indigo-400 w-6' : 'bg-slate-700 w-2 hover:bg-slate-600'}`}
                  />
                ))}
              </div>
              <span className="text-slate-500 text-xs font-medium w-14">{studyPage + 1} / {MIRAS_PANELS.length}</span>
              <button
                onClick={nextPage}
                disabled={isLastPage}
                className="text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed font-bold text-sm px-3 py-1.5 transition-colors"
