...
import { CreditCard, BookOpen, CheckCircle, XCircle, X } from 'lucide-react';
import {
  CREDIT_CARD_CASES,
  USAGE_OPTIONS,
  GLOSSARY_TERMS,
  ...
  type CreditCardCase,
  type UsageAnswer,
} from './data/creditCardAwarenessData';

interface CreditCardAwarenessProps {
...
export default function CreditCardAwareness({ onComplete, onBack }: CreditCardAwarenessProps) {
  const [stage, setStage] = useState<Stage>('intro');
  const [shuffleEnabled, setShuffleEnabled] = useState(false);
  const [orderedCases, setOrderedCases] = useState<CreditCardCase[]>(CREDIT_CARD_CASES);
  const [caseIndex, setCaseIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [correctCount, setCorrectCount] = useState(0);
  const [wrongCount, setWrongCount] = useState(0);
  const [history, setHistory] = useState<AnswerRecord[]>([]);
  const [selected, setSelected] = useState<UsageAnswer | null>(null);
  ...
  const handleStart = () => {
    setOrderedCases(shuffleEnabled ? shuffleCases(CREDIT_CARD_CASES) : CREDIT_CARD_CASES);
    setCaseIndex(0);
    setScore(0);
    setCorrectCount(0);
    setWrongCount(0);
  ...
  const handleSelect = (answer: UsageAnswer) => {
    if (confirmed) return;
    setSelected(answer);
  };
  ...
  const handleConfirm = () => {
    ...
    const isCorrect = selected === currentCase.correctAnswer;
    if (isCorrect) {
      setScore(prev => prev + POINTS_PER_CASE);
    ...
    setHistory(prev => [...prev, {
      ...
      isCorrect,
      correctFeedback: currentCase.correctFeedback,
      wrongFeedback: currentCase.wrongFeedback,
    }]);
    setConfirmed(true);
  };
      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div className="bg-slate-800/60 rounded-2xl p-5 border border-slate-600/50 mb-6 max-w-3xl mx-auto">
          ...
          <ul className="space-y-2 text-sm text-slate-300">
            {[
              `Oyunda 7 vaka bulunur. Her vakada bir kredi kartı kullanım senaryosunu okuyup "Bilinçli Kullanım" ya da "Hatalı Kullanım" olarak sınıflandırırsın.`,
              `Bir seçim yapmadan sonraki vakaya geçemezsin. Cevabını onayladıktan sonra seçimini değiştiremezsin.`,
              `Her doğru cevap ${POINTS_PER_CASE} puan değerindedir, toplam ${MAX_SCORE} puandır. Yanlış cevaplar puan kaybettirmez.`,
              `Sonuç ekranında ham puanın yanında (doğru sayısı / 7) × 100 formülüyle hesaplanan başarı yüzden de gösterilir.`,
  ...
  if (stage === 'finished') {
    ...
    const successRate = Math.round((correctCount / orderedCases.length) * 100);
    return (
        ...
        <div className={`bg-gradient-to-br ${perf.bg} rounded-3xl overflow-hidden shadow-2xl border border-slate-700`}>
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              ...
              <div className="bg-slate-800 rounded-xl p-3 border border-slate-700 text-center">
                <p className="text-xl sm:text-2xl font-black text-blue-400">{history.length} / {orderedCases.length}</p>
                <p className="text-slate-400 text-[10px] sm:text-xs mt-0.5">Tamamlanan Vaka</p>
              </div>
            </div>
            ...
            <div className="space-y-3 mb-6">
              ...
              {history.map((record, idx) => {
                const correctLabel = USAGE_OPTIONS.find(o => o.id === record.correctAnswer)?.label;
                const selectedLabel = USAGE_OPTIONS.find(o => o.id === record.selected)?.label;
                return (
                  <div key={idx} className={`flex items-start gap-3 p-3 rounded-xl border ${record.isCorrect ? 'bg-emerald-900/30 border-emerald-700/40' : 'bg-red-900/30 border-red-700/40'}`}>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-slate-300 text-xs mb-0.5">{record.title}</p>
                      <p className="text-white text-sm font-medium">Seçtiğin: {selectedLabel}</p>
                      {!record.isCorrect && (
                        <p className="text-emerald-300 text-xs mt-0.5">Doğrusu: {correctLabel}</p>
                      )}
                      <p className="text-slate-400 text-xs leading-relaxed mt-1">
                        {record.isCorrect ? record.correctFeedback : record.wrongFeedback}
                      </p>
                    </div>
                    <span className={`text-sm font-black flex-shrink-0 ${record.isCorrect ? 'text-emerald-400' : 'text-slate-500'}`}>
                      {record.isCorrect ? `+${POINTS_PER_CASE}` : '+0'}
                    </span>
                  </div>
                );
              })}
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      <div className="flex items-start gap-4">
        ...
        {/* ── Left Panel: Kredi Kartı Sözlüğü (sticky, desktop) ── */}
                ...
                >
                  ...
                  <div className="flex flex-col gap-3">
                    {USAGE_OPTIONS.map((option, idx) => {
                      ...
                      if (confirmed && isCorrectOption) btnClass = 'bg-emerald-900/60 border-emerald-500 text-emerald-300';
                      else if (confirmed && isSelected && !isCorrectOption) btnClass = 'bg-red-900/60 border-red-500 text-red-300';
                      else if (confirmed) btnClass = 'bg-slate-800 border-slate-700 text-slate-500 opacity-60';
                      ...
                      return (
                        ...
                        >
                          <span>
                            <span className="font-black mr-2">{idx === 0 ? 'A)' : 'B)'}</span>
                            {option.label}
                          </span>
                          {confirmed && isCorrectOption && <CheckCircle className="w-5 h-5 text-emerald-400 flex-shrink-0" />}
                          {confirmed && isSelected && !isCorrectOption && <XCircle className="w-5 h-5 text-red-400 flex-shrink-0" />}
                        </motion.button>
                      );
                    })}
                  </div>
                        ...
                        <div className={`rounded-xl px-5 py-4 mb-4 flex items-start gap-3 ${
                          ...
                          <div>
                            <p className={`text-sm font-bold mb-1 ${lastRecord.isCorrect ? 'text-emerald-300' : 'text-red-300'}`}>
                              {lastRecord.isCorrect ? 'Doğru! +10 puan kazandın.' : `Yanlış cevap. +0 puan. Doğrusu: ${USAGE_OPTIONS.find(o => o.id === lastRecord.correctAnswer)?.label}`}
                            </p>
                            <p className="text-slate-300 text-sm leading-relaxed">
                              {lastRecord.isCorrect ? lastRecord.correctFeedback : lastRecord.wrongFeedback}
                            </p>
                          </div>
                        </div>
                        <motion.button
