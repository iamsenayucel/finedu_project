...
import { CreditCard, Clock, CheckCircle2, XCircle, AlertTriangle, Quote } from 'lucide-react';
import {
  GAME_DURATION_MS,
...
export default function DebtCreditAssessmentGame({ onComplete, onBack }: DebtCreditAssessmentGameProps) {
  ...
  const buildResults = (reason: FinishReason, usedMs: number): ExamResults => {
    ...
    const totalWrong = SECTION1_QUESTIONS.length + SECTION2_CASES.length - totalCorrect;
  ...
  const handleAnswer2 = (optionId: 'A' | 'B') => {
    const c = SECTION2_CASES[c2Index];
    if (answers2[c.id] !== null) return;
    setAnswers2((prev) => ({ ...prev, [c.id]: optionId }));
  };

  const handleNext2 = () => {
    if (c2Index < SECTION2_CASES.length - 1) {
      setC2Index((i) => i + 1);
    } else {
      finalizeExam('user');
    }
          ...
          <div className="p-6 sm:p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-slate-400 text-sm leading-relaxed max-w-xl mx-auto">
                Gerçek hayattan finansal senaryolarda hesap yaparak doğru krediyi seç; ardından bir finansal
                danışman gibi düşünerek gördüğün tavsiyeleri borçlanma prensipleriyle değerlendir.
              </p>
            </div>
            ...
            <div className="bg-slate-800/60 rounded-2xl p-5 border border-slate-600/50 mb-6">
              ...
              <ul className="space-y-2 text-sm text-slate-300">
                {[
                  ...
                  `Bölüm 1'de sırayla 3 finansal senaryo gösterilir. Vaka metnini okuyup verilen bilgiler üzerinden hesap yaparak A, B, C seçeneklerinden birini işaretlersin. Bir soruyu cevapladıktan sonra cevabını değiştiremezsin. Her doğru cevap ${POINTS_PER_QUESTION1} puandır.`,
                  `Bölüm 1'in 3 sorusu tamamlanmadan Bölüm 2'ye geçilmez; sorular otomatik olarak sırayla ilerler.`,
                  `Bölüm 2'de bir finansal danışman rolündesin. Sırayla 3 vaka gösterilir; her vakada önce bir kişinin söylediği görüş, ardından iki farklı danışman yorumu (A/B) sunulur. Borçlanma prensiplerine göre finansal açıdan doğru olan yorumu seçersin. Her doğru cevap ${POINTS_PER_QUESTION2} puandır.`,
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-slate-400 text-xs leading-relaxed">Ölçülen kavram: {c.concept}</p>
                    </div>
                    <span className={`text-sm font-black flex-shrink-0 ${r.correct ? 'text-emerald-400' : 'text-slate-500'}`}>+{r.points}</span>
                  </div>
                );
        ...
        <div className="p-6">
          <AnimatePresence mode="wait">
            {section === 1 ? (
              ...
              >
                <div className="flex items-center gap-2 mb-4 flex-wrap">
                  <span className="bg-slate-700 text-slate-200 text-xs font-black px-3 py-1 rounded-full">{SECTION2_CASES[c2Index].title}</span>
                  <span className="ml-auto bg-blue-900/60 text-blue-300 px-3 py-1 rounded-full text-xs font-black border border-blue-700/60">
                    ...
                    <>
                      <div className="bg-slate-800 rounded-2xl p-5 border border-slate-700 mb-5">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">{c.personLabel}</p>
                        </div>
                        <p className="text-slate-200 text-sm sm:text-base leading-relaxed italic">"{c.quote}"</p>
                      </div>
                        ...
                        {revealed && (
                          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="mt-4">
                            ...
                            >
                              ...
                              <p className="text-slate-300 text-xs leading-relaxed">Ölçülen temel kavram: {c.concept}</p>
                            </div>
                            ...
                            <motion.button
                              ...
                              className="w-full mt-4 bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-500 hover:to-purple-500 text-white font-black py-4 rounded-2xl text-lg shadow-lg border-b-4 border-purple-800"
                            >
                              {c2Index < SECTION2_CASES.length - 1 ? 'Sonraki Vaka ➔' : 'Değerlendirmeyi Bitir 🏁'}
                            </motion.button>
                          </motion.div>
                        )}
                      </AnimatePresence>
