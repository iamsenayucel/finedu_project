...
import { Wallet, BookOpen, CheckCircle, XCircle, X } from 'lucide-react';
import {
  ASSET_CASES,
  ASSET_OPTIONS,
  GLOSSARY_TERMS,
  ...
  type AssetCase,
  type AssetAnswer,
} from './data/assetIncomeExpenseAssessmentData';

interface AssetIncomeExpenseAssessmentProps {
...
export default function AssetIncomeExpenseAssessment({ onComplete, onBack }: AssetIncomeExpenseAssessmentProps) {
  const [stage, setStage] = useState<Stage>('intro');
  const [shuffleEnabled, setShuffleEnabled] = useState(false);
  const [orderedCases, setOrderedCases] = useState<AssetCase[]>(ASSET_CASES);
  const [caseIndex, setCaseIndex] = useState(0);
  const [score, setScore] = useState(0);
  const [correctCount, setCorrectCount] = useState(0);
  const [wrongCount, setWrongCount] = useState(0);
  const [history, setHistory] = useState<AnswerRecord[]>([]);
  const [selected, setSelected] = useState<AssetAnswer | null>(null);
  ...
  const handleStart = () => {
    setOrderedCases(shuffleEnabled ? shuffleCases(ASSET_CASES) : ASSET_CASES);
    setCaseIndex(0);
    setScore(0);
    setCorrectCount(0);
    setWrongCount(0);
  ...
  const handleSelect = (answer: AssetAnswer) => {
    if (confirmed) return;
    setSelected(answer);
  };
      ...
      <div className="w-full max-w-5xl mx-auto">
        ...
        <div className="bg-slate-800/60 rounded-2xl p-5 border border-slate-600/50 mb-6 max-w-3xl mx-auto">
          ...
          <ul className="space-y-2 text-sm text-slate-300">
            {[
              `Oyunda 10 vaka bulunur. Her vakada bir eşya veya varlık senaryosunu okuyup "Gelir Oluşturan Varlık" ya da "Gider Oluşturan Varlık" olarak sınıflandırırsın.`,
              ...
              `Sonuç ekranında ham puanın yanında (doğru sayısı / 10) × 100 formülüyle hesaplanan başarı yüzdesi de gösterilir.`,
          ...
          <div className="p-6 sm:p-8">
            ...
            <div className="space-y-3 mb-6">
              ...
              {history.map((record, idx) => {
                const correctLabel = ASSET_OPTIONS.find(o => o.id === record.correctAnswer)?.label;
                const selectedLabel = ASSET_OPTIONS.find(o => o.id === record.selected)?.label;
                return (
                        ...
                        <div className={`rounded-xl px-5 py-4 mb-4 flex items-start gap-3 ${
                          ...
                          <div>
                            <p className={`text-sm font-bold mb-1 ${lastRecord.isCorrect ? 'text-emerald-300' : 'text-red-300'}`}>
                              {lastRecord.isCorrect ? 'Doğru! +10 puan kazandın.' : `Yanlış cevap. +0 puan. Doğrusu: ${ASSET_OPTIONS.find(o => o.id === lastRecord.correctAnswer)?.label}`}
                            </p>
